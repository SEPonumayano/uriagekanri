...
public interface Datalist3Repository extends JpaRepository<Datalist3, Integer>{
	...
	@Query(value = "select id,maindata.nameid,orderdate,snumber,subtitle,items,delivery_designated_date,delivery_date,billing_date,estimated_amount,order_amount,maindata.statusid,remark,delete_flg,client,stname from(maindata join clientname on maindata.nameid=clientname.nameid)join clients on maindata.statusid=clients.statusid where clients.nameid=clientname.nameid AND delete_flg=0 AND subtitle LIKE %:keyword% AND maindata.nameid LIKE %:nameid%  AND maindata.statusid LIKE %:statusid% order by maindata.id ASC",nativeQuery=true)
	Page<Datalist3> searchlist(@Param("keyword")String keyword,@Param("nameid")int nameid, @Param("statusid")int statusid, Pageable pageable);
	...
	@Query(value = "select id,maindata.nameid,orderdate,snumber,subtitle,items,delivery_designated_date,delivery_date,billing_date,estimated_amount,order_amount,maindata.statusid,remark,delete_flg,client,stname from(maindata join clientname on maindata.nameid=clientname.nameid)join clients on maindata.statusid=clients.statusid where clients.nameid=clientname.nameid AND delete_flg=0 AND subtitle LIKE %:keyword% AND maindata.nameid LIKE %:nameid%  order by maindata.id ASC",nativeQuery=true)
	Page<Datalist3> searchKeyNameid(@Param("keyword")String keyword,@Param("nameid")int nameid, Pageable pageable);
